...
class biLSTMSentenceEncoder(nn.Module):
    ...
    def __init__(self, word_vec_mat, max_length, pos_dim, word_embedding_dim=50, pos_embedding_dim=5, hidden_size=230):
        ...
        self.max_length = max_length
        self.embedding = WordVec.SkipGram(word_vec_mat, max_length, pos_dim, word_embedding_dim, pos_embedding_dim)
        # self.encoders = nn.LSTM(self.hidden_size, self.hidden_size)
        self.lstm = nn.LSTM(input_size=self.max_length, hidden_size=hidden_size, bidirectional=True)
        self.fc = nn.Linear(hidden_size * 2, max_length)
